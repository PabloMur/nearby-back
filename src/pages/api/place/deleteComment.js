...
export default async function handler(req, res) {
  try {
    ...
    const updatedComments = algoliaComments.comments.filter(comment => comment.id !== commentId);

    // Actualiza la lista de comentarios en Algolia
    await algoliaDB.updateObject(placeId, { comments: updatedComments }, (err, content) => {
        if (err) {
            console.error(err);
            return res.status(500).json({
                error: "Error al actualizar la lista de comentarios en Algolia",
                details: err.message,
            });
        }
        console.log('Lista de comentarios actualizada correctamente en Algolia');
    });

    return res.json({ commentDeleted: true, message: 'Comentario eliminado correctamente' });
    } catch (error) {
